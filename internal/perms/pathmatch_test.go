...
// These tests pin the PATH-aware absolute-path matching
// asymmetry described in DESIGN.md: Allow/Ask/SoftAsk match an
// absolute-path command by its bare name only when the path's
// directory is on PATH (the shell would have resolved the bare
// name to that same binary), while Deny strips the basename
// unconditionally so an absolute path can never slip past a
// bare-name deny. They inject PathDirs directly, so unlike the
// bash integration tests they don't depend on the runner's
// real PATH.
...
// pathPerms builds a Permissions with one source holding the
// given Allow and Deny command patterns and the given PATH
// directories.
func pathPerms(
...
func TestDenyStripsBasenameRegardlessOfPath(t *testing.T) {
	...
	// Even though /tmp/evil is off PATH, Deny strips the
	// basename so the absolute path still hits curl:*.
	if got := pathDecide(
...
func TestDenyBeatsAllowForAbsolutePath(t *testing.T) {
	// Same source, same command in both Allow and Deny:
	// within-source precedence makes Deny win, and it does so
	// for an absolute path too.
	p := pathPerms(t,
		[]string{"/usr/bin"},
