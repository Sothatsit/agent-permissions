...
func TestValidateExternalPresetsRejectsRuleOwnedPatterns(
	...
	tests := []struct {
		...
		{"no-arg normalised option", "git --no-pager branch:*", "git branch"},
		{"mixed normalised options", "git --no-pager --git-dir /a tag:*", "git tag"},
		{"attached normalised option", "git --git-dir=/a branch:*", "git branch"},
		{"globbed attached option", "git --work-tree=* remote:*", "git remote"},
