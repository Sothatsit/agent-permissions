...
// TestEmbeddedParses ensures every shipped JSON file in presets/*.json embeds
// and parses cleanly. A parse error would mean the binary ships with broken
// data, so this is build-time-grade in spirit even though it runs as a unit
// test.
...
// TestByNameKnownPreset spot-checks lookup against a preset that's been shipped
// since day one.
