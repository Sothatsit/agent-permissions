#!/usr/bin/env bash
#
# Integration tests for agent-permissions subcommands other than claude-hook:
# setup, install, presets list, rules list, check, and validate. Also asserts
# that the previously-shipped `presets enable` and `presets disable` subcommands
# now fail with a helpful message. Each test runs in an isolated HOME so the
...
# Each test gets its own isolated HOME so writes to ~/.agents/ and ~/.claude/
# don't leak between tests or touch the developer's real files.
_sc_tmpdir=$(mktemp -d)
add_exit_hook _sc_cleanup
_sc_cleanup() { rm -rf "$_sc_tmpdir"; }

# Some usage-error and cwd-specific cases call the binary directly, so isolate
# the whole suite from inherited policy.
...
# Strip ANSI colours that some terminals add to subcommand output so assertions
# don't have to match them.
_sc_run() {
    local h="$1"
...
# --force is also the repair path for a malformed config. It must overwrite the
# file without trying to load it first.
...
# setup reports enforced policy separately and says preset selection cannot
# remove it.
...
# setup validates policy before writing. A semantic error must not leave behind
# a starter file after reporting failure.
...
# Default home - no preset selection; every shipped preset should appear under
# the "Enabled" group.
h=$(_fresh_home)
...
# With disabled-presets in a config, the corresponding preset is grouped under
...
# The list is an effective-policy view, so it must reject the same semantic
# errors as the hook and validate commands.
_sc_preset_dirs="$semantic_bad_dir"
h=$(_fresh_home)
rc=0
...
# enable / disable subcommands were removed - verify they fail with a helpful
# message.
...
# Merges into an existing Bash matcher rather than adding a duplicate top-level
# entry.
h=$(_fresh_home)
mkdir -p "$h/.claude"
cat > "$h/.claude/settings.json" <<'EOF'
...
# Refuses to write through a symlink and leaves the real file untouched.
h=$(_fresh_home)
...
# Refuses when PreToolUse exists in an unexpected shape rather than destroying
# it.
h=$(_fresh_home)
mkdir -p "$h/.claude"
cat > "$h/.claude/settings.json" <<'EOF'
...
# Malformed entries in permissions.json should surface as a Warnings: section
...
# Malformed entries -> exit 2 (validate now returns an error rather than calling
# os.Exit(1) directly, so main's normal error path applies). Lists them and
# quotes the bad entry.
h=$(_fresh_home)
mkdir -p "$h/.agents"
cat > "$h/.agents/permissions.json" <<'EOF'
{
...
# Unknown rule ID in a user .agents config -> exit 2. The config also has a
# malformed pattern, so this doubles as a check that validate reports every
# problem in one pass rather than bailing on the first.
h=$(_fresh_home)
mkdir -p "$h/.agents"
cat > "$h/.agents/permissions.json" <<'EOF'
{
...
# Unknown preset name in disabled-presets -> exit 2, the same silent-no-op class
# as an unknown rule ID.
h=$(_fresh_home)
mkdir -p "$h/.agents"
cat > "$h/.agents/permissions.json" <<'EOF'
...
# A known enforced preset in disabled-presets is not a typo, but it is still an
# ineffective override and must be reported.
_sc_enforced_preset_dirs="$enforced_dir"
h=$(_fresh_home)
mkdir -p "$h/.agents"
echo '{"disabled-presets":["dug-locked"]}' \
...
# Unknown Rules in external presets are load errors, not silent no-ops. This
# covers the same command used by the deployment smoke check.
...
# cwd == $HOME: the project and global agent paths are the same file. Resolve
# must dedup it (keeping the higher-precedence project entry), so one malformed
# entry is reported once, not twice. Run from $HOME itself, not the isolated
# project dir _validate_run uses.
h=$(_fresh_home)
mkdir -p "$h/.agents"
cat > "$h/.agents/permissions.json" <<'EOF'
{"Allow": {"Commands": {":*": ""}}}
